...
def printer(listName, list):
    runningCost = ""
    for index, cost in enumerate(list):
        runningCost = runningCost + f"(Year: {index + 1},Cost: {cost}); "
    print(f"{listName}: {runningCost}")
...
def npv_of_project(startupcost, listofrunningcosts, listofrevnues, saleofbusiness, discountrate):
    ...
    print(f"Start-up costs: {startupcost}")
    printer("Running costs", listofrunningcosts)
    printer("Revenue costs", listofrevnues)
    print(f"Business sales price: {saleofbusiness}")
